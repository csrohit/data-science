from proj.circle import InsufficientException, MobileInventory
import pytest
class TestingInventoryCreation:
    ...
    def test_creating_inventory_with_negative_value(self):
        with pytest.raises(ValueError) as error:
            mi = MobileInventory({'iPhone Model X': -45, 'Xiaomi Model Y': 1000, 'Nokia Model Z': 25})
            ex = error.exception
            assert ex.message == "No. of mobiles must be a positive integer"
...
class TestInventoryAddStock:
    ...
    def test_add_new_stock_as_dict(self):
        ...
        assert self.comp_dict(self.inventory.balance_inventory, {'iPhone Model X':150, 'Xiaomi Model Y': 3000, 'Nokia Model Z':25, 'Nokia Model A':10})
    ...
    def test_add_new_stock_with_numeric_keys(self):
        with pytest.raises(ValueError) as error:
            mi = self.inventory.add_stock({1:'iPhone Model A', 2:'Xiaomi Model B', 3:'Nokia Model C'})
            ex = error.exception
            assert ex.message == "Mobile model must be a string"

    def test_add_new_stock_with_nonnumeric_keys(self):
        with pytest.raises(ValueError) as error:
    ...
    def test_add_stock_with_float_values(self):
        with pytest.raises(ValueError) as error:
